...
const Dithering = memo(function DitheringImpl({
  ...
  const paletteList = (colors.length ? colors : ["#00b2ff"]).slice(0, MAX_DITHER_COLORS);
...
export default function DitherEffect(props: DitherEffectProps) {
  ...
  useEffect(() => {
    ...
    const apply = () => {
      if (!mount) {
        mount = (el.querySelector("[data-dither-shader]") as any)?.paperShaderMount;
        if (!mount) return;
      }
  ...
  const shaderStyle = useMemo<React.CSSProperties>(() => ({ width: "100%", height: "100%" }), []);
